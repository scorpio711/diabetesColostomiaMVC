...
async function consultarProfesionales(profesion = null) {
    try {
        const url = `${location.origin}/public/api/profesionales`;
        const resultado = await fetch(url);
        const profesionales = await resultado.json();

        // Filtrar por profesión si se proporciona
        const profesionalesFiltrados = profesion
            ? profesionales.filter(profesional => profesional.profesion === profesion)
            : profesionales;
...
function mostrarProfesionales(profesionales) {
    // Seleccionar el contenedor y limpiarlo antes de agregar nuevos elementos
    const contenedor = document.querySelector("#profesionales");
    contenedor.innerHTML = ""; // Borra los elementos previos

    profesionales.forEach((profesional) => {
        const { id, nombre, profesion, imagenUsuario } = profesional;

        // Crear el <div> principal con las clases requeridas
        ...
        divElement.dataset.idProfesional = id;
        divElement.onclick = function () {
            seleccionarProfesional(profesional);
        };

        // Crear el contenedor interior para centrar los elementos
        const innerDiv = document.createElement("div");
        innerDiv.classList.add("flex", "flex-col", "items-center", "py-5");

        // Crear la imagen del profesional
        const imgElement = document.createElement("img");
        imgElement.classList.add("w-24", "h-24", "mb-3", "rounded-full", "shadow-lg");
        imgElement.src = `/public/imagenesUsuarios/${imagenUsuario}` || "/public/build/img/avatar.png"; // Ruta de imagen por defecto
        imgElement.alt = `${nombre} image`;

        // Crear el nombre del profesional
        const h5Element = document.createElement("h5");
        h5Element.classList.add("mb-1", "text-xl", "font-medium", "text-gray-900", "dark:text-white");
        h5Element.textContent = nombre;

        // Crear el campo de especialidad del profesional
        const spanElement = document.createElement("span");
        spanElement.classList.add("text-sm", "text-gray-500", "dark:text-gray-400");
        spanElement.textContent = profesion;

        // Agregar todos los elementos al contenedor interior
        innerDiv.appendChild(imgElement);
        innerDiv.appendChild(h5Element);
        innerDiv.appendChild(spanElement);

        // Agregar el contenedor interior al <div> principal
...
function seleccionarProfesional(profesional) {
    ...
    const { profesion } = cita;

    // Identificar al elemento al que se le da clic
    const divProfesional = document.querySelector(`[data-id-profesional="${id}"]`);
    const hElement = divProfesional.querySelector('h5');

    // Desseleccionar todos los profesionales
    document.querySelectorAll('[data-id-profesional]').forEach(div => {
        const h = div.querySelector('h5');
        h.classList.remove("text-purple-600");
    ...
    divProfesional.classList.remove("border-gray-200");
    divProfesional.classList.add("border-purple-600");

    // Actualizar el objeto cita con el nuevo profesional
    cita.profesionales = [profesional];
    console.log(cita);
}
...
function seleccionarServicio(servicio) {
    ...
    // Desseleccionar todos los servicios
    document.querySelectorAll('[data-id-servicio]').forEach(div => {
        const h = div.querySelector('h5');
        h.classList.remove("text-purple-600");
        h.classList.add("text-gray-900");
        div.classList.remove("border-purple-600");
    ...
    // Actualizar el objeto cita con el nuevo servicio
    cita.servicios = [servicio];
    consultarProfesionales(cita.servicios[0].profesionales);
    console.log(cita);
}
